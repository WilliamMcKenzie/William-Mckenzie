...
function animate() {
    ...
    for (let i = 0; i < canvas.width; i += 1) {
        ...
        if (i < cursorX) {
            ctx.fillRect(pixellate(i), pixellate(baseHeight + leftStretch * Math.sin(i) + (cursorY - baseHeight)/(leftStretch / 100)), pixel, pixel)
        }
        else {
            ctx.fillRect(pixellate(i), pixellate(baseHeight + rightStretch * Math.sin(i) + (cursorY - baseHeight)/(leftStretch / 100)), pixel, pixel)
        }
    }
